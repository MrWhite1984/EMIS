...
{
    class RG_CanceledSupplies
    {
        public async static Task<string> GenerateReport(string path)
        {
            List<string[]> result = DataLoader_CanceledSupplies.GetAllCanceledSupplies();
            ...
            doc.Tables[1].Rows[1].Range.Text = $"Список отмененных поставок на {DateTime.Now}\n";
            ...
            doc.SaveAs(path + $@"\ОтчетПоОтмененнымПоставкам({DateTime.Now.Day}_{DateTime.Now.Month}_{DateTime.Now.Year}_{DateTime.Now.Hour}_{DateTime.Now.Minute}_{DateTime.Now.Second}).doc");
            doc.Close();
            word.Quit();
